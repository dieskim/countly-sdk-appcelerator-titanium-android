...
#define TAG "count.ly.messaging"

using namespace v8;

static Persistent<Object> bindingCache;

static void TitaniumCountlyAndroidMessaging_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TitaniumCountlyAndroidMessaging.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::TitaniumCountlyAndroidMessagingBindings::lookupGeneratedInit(
...
static void TitaniumCountlyAndroidMessaging_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TitaniumCountlyAndroidMessaging_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TitaniumCountlyAndroidMessaging_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::TitaniumCountlyAndroidMessagingBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TitaniumCountlyAndroidMessagingBinding = {
	"count.ly.messaging",
	TitaniumCountlyAndroidMessaging_init,
	TitaniumCountlyAndroidMessaging_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_count_ly_messaging_TitaniumCountlyAndroidMessagingBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("count.ly.messaging", &TitaniumCountlyAndroidMessagingBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::TitaniumCountlyAndroidMessagingBindings::lookupGeneratedInit));
}
